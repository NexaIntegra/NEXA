(() => {
  ...
  async function openReader(id) {
    ...
    window.NEXA_ACTIVE_SUMMARY = { summary, exam, pdfUrl: pdfUrl || '' }; $('#readerFavorite').dataset.id = id; $('#readerFavorite').textContent = favorites.includes(id) ? '★ Favoritado' : '☆ Favoritar'; $('#readerDialog').showModal();
    if (pdfUrl && window.pdfjsLib) try { const pdf = await pdfjsLib.getDocument(pdfUrl).promise; pdfState = { pdf, page: 1, scale: 1 }; window.NEXA_ACTIVE_SUMMARY.pdf = pdf; await renderPdf(true); } catch { $('#pdfCanvasHost').innerHTML = '<p>Não foi possível abrir a prévia. Use o botão para baixar o PDF.</p>'; }
  }
