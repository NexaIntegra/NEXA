(() => {
 ...
 const examAction = event.target.closest('[data-exam-action]'); if (examAction) { const exam = exams.find(item => item.id === examAction.dataset.id); if (examAction.dataset.examAction === 'edit') { $('#examId').value = exam.id; $('#examSubject').value = exam.subject; $('#examTitle').value = exam.title; $('#examClassTime').value = exam.classTime; } if (examAction.dataset.examAction === 'delete') try { await nexaApi.deleteExam(exam.id); await load(); } catch { $('#examMessage').textContent = 'Esta prova possui resumos vinculados e não pode ser removida.'; } } });
